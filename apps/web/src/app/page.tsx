...
import { Activity, ShieldCheck, AlertTriangle, Clock, Server, Lock, Globe, Cpu, ArrowUpRight, Sparkles } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  // Sub-second telemetry heartbeat simulation (~1s update)
  useEffect(() => {
  ...
  const monitors = [
    ...
    { id: 'mon-102', name: 'Stripe Webhook Receiver', type: 'HTTP', target: 'https://payments.altix.io/webhooks', status: 'UP', latency: 42, uptime: 100.0 },
    { id: 'mon-103', name: 'Primary SSL Certificate', type: 'SSL', target: 'altix.io (74d)', status: 'UP', latency: 12, uptime: 100.0 },
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="p-6 space-y-6 flex-1 overflow-y-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-card p-6 rounded-2xl border border-white/10 relative overflow-hidden">
            ...
            <div className="space-y-1 z-10">
              ...
              <p className="text-sm text-altix-muted">
                Monitoramento contínuo em tempo real. Sub-segundo pulse ativado (~1s latency broadcast).
              </p>
            </div>
            ...
            <div className="flex items-center gap-6 z-10">
              <div className="flex flex-col text-right">
                ...
                <span className="text-xl font-bold font-mono text-altix-green">
                  {pulseCount.toLocaleString('pt-BR')}
                </span>
              </div>
              <div className="flex flex-col text-right">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="glass-card glass-card-hover p-5 rounded-2xl space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="p-2 rounded-xl bg-altix-green/10 text-altix-green">
                  <Activity className="w-4 h-4" />
                </div>
              </div>
              <div className="flex items-baseline justify-between">
                <span className="text-3xl font-extrabold font-mono">5</span>
                <span className="text-xs text-altix-green font-semibold flex items-center">
                  +2 este mês <ArrowUpRight className="w-3 h-3 ml-0.5" />
                </span>
              </div>
              <div className="text-[11px] text-altix-muted flex gap-2">
                <span>3 HTTP</span> • <span>1 SSL</span> • <span>1 TCP</span>
              </div>
            </div>
            ...
            <div className="glass-card glass-card-hover p-5 rounded-2xl space-y-3">
              ...
              <div className="flex items-baseline justify-between">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-altix-green/20 text-altix-green font-bold">
                  80% ONLINE
                </span>
              </div>
            ...
            {/* Card 3 */}
            <div className="glass-card glass-card-hover p-5 rounded-2xl space-y-3 border-altix-offline/30">
              <div className="flex items-center justify-between">
                <span className="text-xs text-altix-muted font-medium">Incidentes Ativos</span>
                <div className="p-2 rounded-xl bg-altix-offline/10 text-altix-offline">
                  <AlertTriangle className="w-4 h-4" />
                </div>
              </div>
              <div className="flex items-baseline justify-between">
                <span className="text-3xl font-extrabold font-mono text-altix-offline">1</span>
                <span className="text-xs px-2 py-0.5 rounded-full bg-altix-offline/20 text-altix-offline font-bold animate-pulse">
                  CRÍTICO
                </span>
              </div>
              <div className="text-[11px] text-altix-offline flex items-center gap-1">
                <span>ns1.altix.io Down</span>
              </div>
            </div>
            ...
            <div className="glass-card glass-card-hover p-5 rounded-2xl space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="p-2 rounded-xl bg-altix-warning/10 text-altix-warning">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  <Clock className="w-4 h-4 text-altix-green" /> Latência da Infraestrutura em Tempo Real
                </h3>
                <p className="text-xs text-altix-muted">Tempo médio de resposta (TTFB + DNS + TCP)</p>
              </div>
              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 rounded-full bg-altix-green pulse-dot-online" />
                <span className="text-xs font-mono text-altix-green font-bold">{liveLatency} ms</span>
              </div>
            </div>
          ...
          <div className="glass-card p-6 rounded-2xl space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <Server className="w-4 h-4 text-altix-green" /> Ativos Sob Monitoramento ({monitors.length})
              </h3>
                    ...
                    <tr key={mon.id} className="hover:bg-white/5 transition-colors">
                      <td className="py-3 px-4">
                        {mon.status === 'UP' ? (
                          <div className="flex items-center gap-2">
                            <span className="w-2.5 h-2.5 rounded-full bg-altix-green pulse-dot-online" />
                            <span className="font-bold text-altix-green font-mono">ONLINE</span>
                          </div>
                        ) : (
                          <div className="flex items-center gap-2">
                            <span className="w-2.5 h-2.5 rounded-full bg-altix-offline pulse-dot-offline" />
                            <span className="font-bold text-altix-offline font-mono">OFFLINE</span>
                          </div>
                        )}
                      </td>
                      <td className="py-3 px-4 font-semibold text-white">{mon.name}</td>
                      <td className="py-3 px-4">
                        <span className="px-2 py-0.5 rounded bg-white/10 font-mono text-[10px]">{mon.type}</span>
                      </td>
                      ...
                      <td className="py-3 px-4 w-48">
                        <UptimeBar days={30} uptimePercent={mon.uptime} />
                      </td>
                    </tr>
                  ))}
                </tbody>
